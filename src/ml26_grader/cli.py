...
from .ingest.batch import discover_batch_submissions, extract_submission_zip
...
def build_parser() -> argparse.ArgumentParser:
    ...
    inspect_q4 = subparsers.add_parser(
        "inspect-q4",
        help="Run Q4 artifact validation and deterministic result checks.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "inspect-q4":
        ...
        result = pipeline.evaluate(args.submission, dataset_name=args.dataset)
        print(render_json_document(result))
        return 0
